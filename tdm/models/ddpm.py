...
class DDPM(pl.LightningModule):
    ...
    def __init__(
        ...
        # Condition model
        self.cond_model = getattr(torchvision.models, cond_model)(pretrained=True)
        if unet_config['in_ch'] == 1:
            conv_weight = self.cond_model.conv1.weight
            conv_weight = torch.mean(conv_weight, dim=1, keepdim=True)
            self.cond_model.conv1 = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3, bias=False)
    ...
    def training_step(self, batch, batch_idx):
        x, aim, cond, *_ = batch  # Assume dataset returns (image, aim, feature)
        x = x.to(self.device)
    ...
    def _sample(self, shape, aim, cond=None, num_steps=None):
        ...
        if cond is not None:
            ...
            raise ValueError("Conditioning features must be provided for sampling.")
